...
Set objFile = objFSO.OpenTextFile (strFilePath & "\ExStudents Last Logon " & strLDate & ".csv",ForAppending,True)
...
Set objList4 = CreateObject("Scripting.Dictionary")
Set objList5 = CreateObject("Scripting.Dictionary")
...
'*******************************************************************
  ...
  objCommand.CommandText = "<LDAP://" & sDCs(k) & "/ou=exstudents,ou=mc users,dc=manchester,dc=edu" & ">;(ObjectCategory=User);Name,LastLogon,givenName,sn,ipPhone,employeeNumber;subtree"
  ...
  Else
    ...
    Do Until objRecordSet.EOF
	  ...
	  strEmployeeNumber = objRecordSet.Fields("employeeNumber")
      On Error Resume Next
      Err.Clear
      Set lngDate = strDate
      If Err.Number <> 0 Then
      ...
      Else
		...
		objList4.Add strAdsPath, strIPPhone
		objList5.Add strAdsPath, strEmployeeNumber
      End If
      objRecordSet.MoveNext
    Loop
  End If
...
'* Output latest LastLogon date for each computer.
  ...
  For Each strUser In objList
    ...
    Call  VBOut(strUser,objList(strUser),objList2(strUser),objList3(strUser),objList4(strUser),objList5(strUser))
...
Function VBOut(strPC,strTime,strFN,strLN,strIPPhone,strEmployeeNumber)
...
'* Write to output file
   ...
   objFile.WriteLine Trim(strUserName) & "," & strFN & "," & strLN & "," & strIPPhone & "," & strLogonTime & "," & strEmployeeNumber
