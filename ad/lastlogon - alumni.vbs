...
Set objFile = objFSO.OpenTextFile (strFilePath & "\Alumni Last Logon " & strLDate & ".csv",ForAppending,True)
...
'*******************************************************************
  ...
  objCommand.CommandText = "<LDAP://" & sDCs(k) & "/ou=alumni,dc=manchester,dc=edu" & ">;(ObjectCategory=User);Name,LastLogon,givenName,sn,ipPhone;subtree"
...
'* Output latest LastLogon date for each computer.
  ...
  For Each strUser In objList
    ...
    Call  VBOut(strUser,objList(strUser),objList2(strUser),objList3(strUser),objList4(strUser))
...
Function VBOut(strPC,strTime,strFN,strLN,strIPPhone)
...
'* Write to output file
   ...
   objFile.WriteLine Trim(strUserName) & "," & strFN & "," & strLN & "," & strIPPhone & "," & strLogonTime
